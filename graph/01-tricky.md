...
9. 有向无环图的最大独立集
   ...
   最后 $H$ 集合就是独立集集合
10. >给定一张二分图, 其最大匹配方案不一定是唯一的. 若任何一个最大匹配方案的匹配边都包括 $(x,y),$ 则称 $(x,y)$ 为二分图匹配的必须边. 若 $(x,y)$ 至少属于一个最大匹配的方案, 则称 $(x,y)$ 为二分图匹配的可行边.
   ...
   2. $(x,y)$ 是非匹配边, 设当前 $x$ 与 $u$ 匹配, $y$ 与 $v$ 匹配, 连接边 $(x,y)$ 后, 节点 $u,v$ 失去匹配. 
   ...
   2. 即使当前 $x$ 与 $u$ 匹配, $y$ 与 $v$ 匹配, 连接边 $(x,y)$ 后, 节点 $u,v$ 失去匹配, 我们也不一定要找到从 $u$ 到 $v$ 的增广路, 如果从 $u$ 出发找到另一个非匹配点 $z$ 的增广路, 同样可以得到一组包含 $(x,y)$ 的最大匹配.
