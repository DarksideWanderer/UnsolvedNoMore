...
$$
[x^n]A^k(x)'A(x)
= [x^n]\big(kA^k(x)A'(x)\big)
= \sum_{i=0}^n (i+1)\,res_{i+1}a_{n-i}
= k\sum_{i=0}^n (i+1)a_{i+1}res_{n-i}
$$
...
[x^{n+1}]G(x)
= \frac{
k\displaystyle\sum_{i=0}^{m-1}(i+1)a_{i+1}[x^{n-i}]G(x)
- \displaystyle\sum_{i=n-m}^{n-1}(i+1)[x^{i+1}]G(x)a_{n-i}
}{
(n+1)a_0
}
$$
